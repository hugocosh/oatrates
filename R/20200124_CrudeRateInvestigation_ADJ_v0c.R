...
# Crude rate investigation 24/01/2020 ADJ
# This code investigates the difference in confidence interval calculation for three different methods, exact Poisson, Byar's and exact Chi square
# The observatory currently uses Wilson's method for all crude rate calculations
...
Data <- data.frame(Numerator = c(5, 10 , 20, 50, 100, 200), Denominator = 5000)

Data <- Data %>%
        mutate(CrudeRate = (Numerator/Denominator) * 1000)


# Exact Poisson confidence interval (currently used by observatory)

# Need to load CI data from statistical calculation sheet
# \\ryt6bsrvfil0002\observatory\Health Intelligence\Method\Statistical methods\Calculation sheets\20130112_Methods_03_CrudeRateCIs_GD.xls

CIdata <- read_excel("20130112_Methods_03_CrudeRateCIs_GD.xls", sheet = "CI data"
                     , col_names = FALSE)

# Drop columns 4 and 5 and rename columns 1-3
CIdata$...4 <- NULL
CIdata$...5 <- NULL
...
# Calculate CIs dependent on number of events <= 100 or > 100

Under <- Data %>%
  filter(Numerator <= 100) %>%
left_join(CIdata, by = c("Numerator" = "X")) %>%
  mutate(LCL = (Xl/Denominator) * 1000
         , UCL = (Xu/Denominator) * 1000)

Over <- Data %>%
  filter(Numerator > 100) %>%
  mutate(Xl = ((1.96/2) - sqrt(Numerator)) ^2
           ,Xu = ((1.96/2) + sqrt(Numerator + 1)) ^2) %>%
  mutate(LCL = (Xl/Denominator) * 1000
         , UCL = (Xu/Denominator) * 1000)

Poisson <- bind_rows(Under, Over)


# Byar's method - Method taken from https://fingertips.phe.org.uk/profile/guidance


Byar <- Data %>%
  mutate(Xl = Numerator * (1 - 1/(9 * Numerator) - 1.96/(3 * sqrt(Numerator))) ^ 3
         , Xu = (Numerator + 1) * (1 - 1/(9 * (Numerator + 1)) + 1.96/(3 * sqrt(Numerator + 1))) ^ 3) %>%
  mutate(LCL = (Xl/Denominator) * 1000
         , UCL = (Xu/Denominator) * 1000)



# Exact chi square method

Test <- qchisq(p = (1 - (0.05/2)), df = (2 * Data$Numerator), lower.tail = FALSE)

Test

Chi <- Data %>%
  mutate(Xl = qchisq(p = (1 - (0.05/2)), df = (2 * Numerator), lower.tail = FALSE) / 2
         , Xu = qchisq(p = (1 - (0.05/2)), df = (2 * Numerator) + 2) / 2
                       ) %>%
  mutate(LCL = (Xl/Denominator) * 1000
         , UCL = (Xu/Denominator) * 1000)
...
################### Now turn these calculations into a function that can be used at any point
